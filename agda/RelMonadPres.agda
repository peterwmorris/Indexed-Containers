...
module RelMonadPres where
  ...
  ηpres {l} I i = record { fiso = {!!}; law1 = {!!}; law2 = {!!} }



{- record { fiso = λ {A} → iso (λ a → _ , λ p → subst A p a) (split (λ _ a → a refl)) (ext (λ a → cong₂ _,_ refl (iext (ext (foo a))))) refl; law1 = λ {A} {B} {f} → ext (λ a → cong₂ _,_ refl (iext (ext (λ p → trans (subst-removable B p (f a)) (cong₂ (λ i → f {i}) p (sym (subst-removable A p a))))))); law2 = refl }
...
-}
  >>=pres : {l : Level} (I J : Set l) (H : I → ICont J) (F : ICont I) →
             ((λ i → ⟦ H i ⟧) >>=^F ⟦ F ⟧) ≅^F ⟦ H >>=^C F ⟧
  >>=pres {l} I J H (S ◁ P) =   record { fiso = {!!}; law1 = {!!}; law2 = {!!} }
{-
